...
int main() {
    ...
    std::filesystem::create_directories("results/dynamic_ls_" + std::string((cfg.diffusion_model == IC) ? "IC" : "LT"));

    std::ofstream csv(
        "results/dynamic_ls_" + std::string((cfg.diffusion_model == IC) ? "IC" : "LT") + "/nodes_saved_" + pct.str() + ".csv"
    );
    std::ofstream csv_time(
        "results/dynamic_ls_" + std::string((cfg.diffusion_model == IC) ? "IC" : "LT") + "/time_taken_" + pct.str() + ".csv"
    );
