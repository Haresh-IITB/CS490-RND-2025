...
std::vector<std::pair<int,int>> Run_Dynamic_Vaccination(
    ...
    std::vector<int> current_infected = initial_infected ; // Stores the frontier 
    std::vector<std::pair<int,int>> all_new_vaccines ; // pair of (node, time_id)
    std::vector<bool> infected_mask(G.nodes.size(), false) ; // Stores all the nodes infected so far

    for(int node : initial_infected) 
        infected_mask[node] = true ;
...
int main() {
    ...
    Config cfg;
    if (!load_config("config/config-greedy.txt", cfg)) {
        std::cerr << "Failed to load config/config-greedy.txt\n";
        return 1;
    }
