...
void simulate_infection_spread(
    const Graph &G, 
...
std::vector<int> solve_lp_and_pick_new(
    LPModel &lp, 
    int marginal_budget,
    const std::vector<bool>& pre_vaccinated_mask,
    const std::vector<bool>& already_infected_mask
...
std::vector<std::pair<int,int>> run_rolling_horizon_strategy(
    ...
    for (size_t t = 0; t < budget_schedule.size(); ++t) {
        ...
        auto samples = sample_ic_live_edge_topologies(base_graph, num_samples_per_step, 42 + t, prob_infect);
